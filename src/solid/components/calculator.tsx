...
const Calculator = (props: CalculatorProps) => {
	...
	/**
	 * 向当前输入添加一个值
	 * 此函数主要用于处理用户输入的字符，并根据一定的逻辑更新当前的输入状态
	 * @param value 要添加的值，通常是一个字符
	 */
	const addValue = (value: string) => {
		...
		// 处理小数点的输入
		if (value === ".") {
			// 如果当前输入为空，或者最后一个字符不是数字，或者已包含小数点，则忽略此次输入
			if (
		...
		const lastValueIsZero = lastValue?.slice(0, 1) === "0";

		if (lastValueIsZero && value === "0" && !lastValue.includes(".")) {
			return;
		}

		// 根据上一个值是否为数字，决定如何处理当前输入
		if (!isNaN(Number(lastValue))) {
			// 如果上一个值是数字，将当前值追加到该数字上
			newValue[inputLength - 1] += value;
		} else {
			// 如果上一个值不是数字，将当前值作为一个新的元素添加到输入列表中
			newValue.push(value);
		}
	...
	const deleteLast = () => {
		const inputArray = currentInput();
		const inputLength = inputArray.length;
		if (!inputLength) {
			return;
		}
		const lastValue = inputArray[inputLength - 1];
		if (!isNaN(Number(lastValue))) {
			inputArray[inputLength - 1] = inputArray[inputLength - 1].slice(0, -1);
		} else {
			inputArray.pop();
		}
		setCurrentInput(inputArray.filter((item) => item !== ""));
	};
	...
	const setValuePercentage = () => {
		...
		const lastValue = inputArray[inputLength - 1];
		if (!isNaN(Number(lastValue))) {
			inputArray[inputLength - 1] = (parseFloat(lastValue) / 100).toString();
			setCurrentInput(inputArray);
		}
	};
